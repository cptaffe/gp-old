...
int calcfit(char expresion[], char *argv[]) {
    ...
    if (strlen(expresion) > maxChroLen) {return 1;}
    fit = 0;
    ...
    if (g == 0) {
        ...
        if (minKeepFitness < 0) {minKeepFit = highFitness -1;}
        else {minKeepFit = minKeepFitness;}
    }
    ...
    if (fit >= minKeepFit) {
        ...
        if (fit > highFitness) {highFitness = fit;}
        size++;
...
void getIndex(char stopChar, char *Astring, int &Aplace, int &Aend) {
    ...
    if (randA <= openParenA) {
        ...
        Aend = Aplace;
        while (Astring[Aend] != ')' && Astring[Aend] != ' ' && Astring[Aend] != '(' && Astring[Aend] != stopChar &&
                Astring[Aend] != 0) {
            Aend++;  // Aend is now the position for end of leaf
        }
        Aplace--; // and backup one to pickup space on front end
